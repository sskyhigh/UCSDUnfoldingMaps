...
public class EarthquakeCityMap extends PApplet {
	...
	private void sortAndPrint(int numToPrint) {
		List<EarthquakeMarker> earthquake = new ArrayList<EarthquakeMarker>();
		for (Marker mark : quakeMarkers) {
			earthquake.add((EarthquakeMarker) mark);
		}
		Collections.sort(earthquake);
		if (numToPrint > earthquake.size()) {
			for (int i = 0; i < earthquake.size(); ++i) {
				System.out.println(earthquake.get(i));
			}
		} else {
